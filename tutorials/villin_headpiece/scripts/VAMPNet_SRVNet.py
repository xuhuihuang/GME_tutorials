...
def TimeLaggedDataset(trajs=None, lagtime=100, normalize=False, mean=None, std=None):
    ...
    _pastdata = _pastdata.astype(np.float32); _futuredata = _futuredata.astype(np.float32)
...
def rao_blackwell_ledoit_wolf(covmatrix=None, num_data=None, device=None):
    """
    ...
    F = (torch.trace(covmatrix) / matrix_dim) * torch.eye(matrix_dim).to(device)
    return (1-rho)*covmatrix + rho*F, rho


def covariance(pastdata=None, futuredata=None, remove_mean=True, device=None):
    ...
    c00, _ = rao_blackwell_ledoit_wolf(covmatrix=c00, num_data=pastdata.shape[0], device=device)
    ctt, _ = rao_blackwell_ledoit_wolf(covmatrix=ctt, num_data=futuredata.shape[0], device=device)
    return c00, c0t, ctt
...
def koopman_matrix(pastdata=None, futuredata=None, epsilon=1e-6, method='cutoff', device=None):
    ...
    c00, c0t, ctt = covariance(pastdata=pastdata, futuredata=futuredata, remove_mean=True, device=device)
...
def reverse_propogator(pastdata=None, futuredata=None, epsilon=1e-6, method='cutoff', device=None):
    ...
    c00, c0t, ctt = covariance(pastdata=pastdata, futuredata=futuredata, remove_mean=True, device=device)
    _, ct0, _ = covariance(pastdata=futuredata, futuredata=pastdata, remove_mean=True,  device=device)
...
def loss_function(pastdata=None, futuredata=None, criterion='SRVNet', epsilon=1e-6, method='cutoff', device=None):
    ...
    if criterion == 'SRVNet':
        rev_propogator = reverse_propogator(pastdata=pastdata, futuredata=futuredata, epsilon=0, method='regularize', device=device)
        _eigenval, _ = torch.linalg.eigh(rev_propogator)
        _score = torch.sum(torch.pow(_eigenval, 2)) + 1


    elif criterion == 'VAMP1':
        koopman_mat = koopman_matrix(pastdata=pastdata, futuredata=futuredata, epsilon=epsilon, method=method, device=device)
...
class deep_projector(object):
    ...
    def partial_fit(self, pastdata, futuredata):
        ...
        if isinstance(futuredata, np.ndarray):
            futuredata = torch.from_numpy(futuredata.astype(np.float32))
        ...
        _loss = loss_function(pastdata=_pastproject, futuredata=_futureproject, criterion=self._network_type, 
                              epsilon=self.__epsilon, method=self.__covariance_method, device=self._device)
        _loss.backward()
        ...
        if self._network_type == 'SRVNet':
            rev_propogator = reverse_propogator(pastdata=_pastproject, futuredata=_futureproject, epsilon=self.__epsilon, 
                                                method=self.__covariance_method, device=self._device)
            _eigenval, _ = torch.linalg.eigh(rev_propogator)
    ...
    def _normalize(self, data, lagtime, device=None):
        ...
        if not self._network_type == "SRVNet":
            ...
            pastdata = pastdata.to(device=default_device); futuredata = futuredata.to(device=default_device)
            pastmean = torch.mean(pastdata, dim=0); futuremean = torch.mean(futuredata, dim=0)
            mean = ((pastmean + futuremean) / 2).cpu().numpy()
            c00, c0t, ctt = covariance(pastdata=pastdata, futuredata=futuredata, remove_mean=True, device=device)
            _, ct0, ctt = covariance(pastdata=futuredata, futuredata=pastdata, remove_mean=True, device=device)
